...
import { Home,Search, UserPlus, MenuIcon, X, ChevronRight, Calendar,Stethoscope } from "lucide-react"
import lungs from "./assets/lungs.png"
import avatar from "./assets/avatar.png"

const tabs = [
  { name: "Home", icon: <Home size={20} />, path: "/accueil-patient" }, // Changed path for patient home
  { name: "Gérer mes symptômes", icon: <Stethoscope size={20} />, path: "/symptomes" },
  { name: "Agenda", icon:<Calendar size={20} />, path: "/agenda" }, // Ensure this path matches your routing
]
...
function NavbarPatient() {
  ...
  const [patientNom, setPatientNom] = useState("") // New state for patient's first name
  const [patientPrenom, setPatientPrenom] = useState("") // New state for patient's last name
  const [openDropdown, setOpenDropdown] = useState(false)
  const dropdownRef = useRef(null)

  // Fetch patient name from localStorage on component mount
  useEffect(() => {
  ...
  const handleLogout = () => {
    // Clear all patient-related items from localStorage
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            >
              ...
              <span className="font-medium text-gray-800 text-sm">
                {patientNom} {patientPrenom} {/* Display patient's name */}
              </span>
            </button>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-gray-100">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-bold text-gray-800">{patientNom} {patientPrenom}</h3> {/* Display patient's name */}
              <p className="text-sm text-gray-500">Patient</p> {/* Changed role to Patient */}
            </div>
          </div>
          <button
            onClick={handleLogout}
